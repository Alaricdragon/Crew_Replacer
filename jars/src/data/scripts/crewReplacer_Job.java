...
public class crewReplacer_Job {
    ...
    public float getCargoSpaceUsed(CargoAPI cargo,String cargoType){
        CrewReplacer_Log.loging(getIntoJobLog(cargo) + "running get Cargo Space Used of type: " + cargoType,this);
        CrewReplacer_Log.push();
        float output = 0;
            ...
            * cargoSpaces = the cargospace per item of a given crew (possibly including defence of said crew.)*/
            ...
            if(power < powerTemp){
                ...
                for(int a = 0; a < sortedID.size(); a++){
                    ...
                    float crewPowerTemp = Crews.get(id).getCrewPower(cargo);
                    ...
                    cargoTemp += Crews.get(id).getCargoSpaceUse(cargo);
                }
                output[0] += cargoTemp;
